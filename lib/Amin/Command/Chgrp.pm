...
sub end_element {
	...
	if (($element->{LocalName} eq $self->{'Spec'}->{'Localname'}) && ($self->command eq $self->command_name)) {
		...
		my $params = $self->param;
          my @doubles = qw(help version silent changes derefence no-dereference recursive verbose);
          my $flag = $self->get_flag($flags, \@doubles);
          #dir check
          if ($dir) { $self->chdir($dir); }
		my $acmd = $self->get_command($self->command_name, $flag, $params);
		my $cmd = $self->amin_command($acmd);
          my $error = "Chgrp Failed for " . join (", ", @$params) . " Reason: $cmd->{ERR}";
          my $success = "Chgrp was successful for " . join (", ", @$params);
