...
sub end_element {
	...
	if (($element->{'LocalName'} eq $self->{'Spec'}->{'Localname'}) && ($self->command eq $self->command_name)) {
		...
		my $log = $self->{Spec}->{Log};
          ...
          my @doubles = qw (help version silent quiet changes dereference no-dereference recursive verbose);
          my $flag = $self->get_flag($flags, \@doubles);
		my $acmd = $self->get_command($self->command_name, $flag, $params);
		my $cmd = $self->amin_command($acmd);
          my $error = "Chown Failed for " . join (", ", @$params) . " Reason: $cmd->{ERR}";
          my $success = "Chown was successful for " . join (", ", @$params);
